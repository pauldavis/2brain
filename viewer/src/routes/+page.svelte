<script lang="ts">
    ...
    const selectedDocumentId = $derived<string | null>(
        selectedDocument?.document.id ?? null,
    );
    let selectedSegmentIndex = $state(0);
    let isLoadingDocument = $state(false);
    let loadError = $state("");
    const initialHydrationStart =
    ...
    const emptySegmentCount = $derived(
        selectedDocument
            ? segments.filter((segment) => !segmentHasContent(segment)).length
            : 0,
    );
    const currentSegment = $derived<Segment | null>(
        visibleSegments[selectedSegmentIndex] ?? null,
    );
    ...
    const transcriptDownloadUrl = $derived<string | null>(
        selectedDocumentId
            ? `${API_BASE}/documents/${selectedDocumentId}/transcript?format=markdown&download=1`
            : null,
    );
    ...
    const selectedDocumentCharCount = $derived(
        selectedDocument
            ? selectedDocument.segments.reduce((total, segment) => {
                  const length =
                      typeof segment.content_markdown === "string"
                          ? segment.content_markdown.length
                          : 0;
                  return total + length;
              }, 0)
            : null,
    );
    const selectedDocumentCharLabel = $derived(
        formatCharacterCount(selectedDocumentCharCount),
    );
    const selectedDocumentSegmentLabel = $derived(
        selectedDocument
            ? formatSegmentCount(selectedDocument.segments.length)
            : null,
    );
    ...
    async function selectDocument(id: string, segmentId?: string | null) {
        ...
        try {
            ...
            viewMode = "sections";
            console.log(
                `[document] id=${id} fetched in ${Math.round(performance.now() - t0)}ms`,
            );
        } catch (error) {
            console.error(error);
            loadError =
            ...
            console.log(
                `[document] id=${id} failed after ${Math.round(performance.now() - t0)}ms`,
                error,
            );
        } finally {
    ...
    async function loadTranscript(force = false) {
        ...
        if (
            transcript &&
            transcript.document.id === selectedDocumentId &&
            !force
        )
            return;
        const t0 = performance.now();
        ...
        try {
            ...
            const res = await fetch(
                `${API_BASE}/documents/${selectedDocumentId}/transcript`,
                { headers },
            );
            if (!res.ok) {
            ...
            transcript = payload;
            console.log(
                `[transcript] document=${selectedDocumentId} fetched in ${Math.round(performance.now() - t0)}ms`,
            );
        } catch (error) {
            console.error(error);
            transcriptError =
                error instanceof Error
                    ? error.message
                    : "Unknown error while loading transcript.";
            console.log(
                `[transcript] document=${selectedDocumentId} failed after ${Math.round(performance.now() - t0)}ms`,
                error,
            );
        } finally {
    ...
    function segmentHasContent(segment: Segment) {
        ...
        if (segment.blocks?.length) {
            return segment.blocks.some(
                (block) => (block.body?.trim()?.length ?? 0) > 0,
            );
        }
        return false;
    }
    ...
    function dedentCode(raw: string, language: string | null): string {
        ...
        for (const line of lines) {
            ...
            minIndent =
                minIndent === null ? indent : Math.min(minIndent, indent);
        }
    ...
    const segmentedContent = $derived(
        splitContent(currentSegment?.content_markdown ?? null),
    );
    const hasDetailedBlocks = $derived(() =>
        (currentSegment?.blocks ?? []).some(
            (block) => block.block_type !== "markdown",
        ),
    );
    ...
    $effect(() => {
        if (selectedSegmentIndex >= visibleSegments.length) {
            selectedSegmentIndex =
                visibleSegments.length > 0 ? visibleSegments.length - 1 : 0;
        }
    });

    $effect(() => {
        if (!pendingSegmentId) return;
        const idx = visibleSegments.findIndex(
            (segment) => segment.id === pendingSegmentId,
        );
        if (idx >= 0) {
...
<div
    class="flex min-h-screen flex-col bg-slate-50 text-slate-900 md:grid md:grid-cols-[320px_1fr]"
>
    <aside
    ...
    >
        <div class="flex h-full flex-col gap-4 p-6">
            ...
            <p class="text-sm text-slate-500">
                Filter and choose a conversation to review.
            </p>
            ...
            {:else}
                <div
                    class="max-h-[60vh] overflow-y-auto pr-1 md:max-h-[calc(100vh-160px)]"
                >
                    <ul class="space-y-2">
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <span
                                            ...
                                            title={`Updated ${formatDate(doc.updated_at)}`}
                                        >
    ...
    <main class="flex min-h-screen flex-col gap-6 overflow-y-auto p-6 lg:p-10">
        ...
        {#if selectedDocument}
            ...
            >
                <div class="card-body space-y-6">
                    <div class="space-y-3">
                        ...
                        >
                            <h2 class="card-title text-3xl font-semibold">
                            ...
                            >
                                ...
                                >
                                {formatShortDate(
                                    selectedDocument.document.updated_at,
                                )}
                            </div>
                        </div>
                        <div
                        ...
                        >
                            ...
                            >
                                {#if selectedDocumentCharLabel}
                                    <span class="badge badge-outline"
                                        >{selectedDocumentCharLabel}</span
                                    >
                                {/if}
                                {#if selectedDocumentSegmentLabel}
                                    <span class="badge badge-outline"
                                        >{selectedDocumentSegmentLabel}</span
                                    >
                                {/if}
                            </div>
                            <div
                            ...
                            >
                                <span class="font-semibold text-slate-500">
                                    {emptySegmentCount} empty segment{emptySegmentCount ===
                                    1
                                        ? ""
                                        : "s"}
                                </span>
                        ...
                        >
                            ...
                            >
                                <span
                                    ...
                                    >Conversation overview</span
                                >
                                <span class="text-xs text-slate-400"
                                    >scrollable summary (Markdown)</span
                                >
                            </header>
                            <div
                                class="markdown max-h-60 overflow-y-auto text-slate-700"
                            >
                                {@html renderMarkdown(
                                    sanitizeSummary(
            ...
            {#if viewMode === "document"}
                <section
                    class="card rounded-lg border border-slate-200 bg-white shadow-xl"
                >
                    <div class="card-body space-y-6">
                        ...
                        {:else}
                            <div class="space-y-2 text-sm text-slate-600">
                                ...
                                <ul class="list-disc space-y-1 pl-5">
                                    <li>
                                        Source system: {selectedDocument
                                            .document.source_system}
                                    </li>
                                    <li>
                                        External ID: {selectedDocument.document
                                            .external_id}
                                    </li>
                                    <li>
                                        Segment count: {selectedDocument
                                            .segments.length}
                                    </li>
                                    <li>
                                        Created: {formatDate(
                            ...
                            <div class="space-y-6">
                                {#each selectedDocument.segments as segment, index}
                                    ...
                                    >
                                        ...
                                        <div
                                            class="markdown text-sm wrap-break-word"
                                        >
                                            {@html renderMarkdown(
                                        ...
                                        {#if segment.assets?.length}
                                            <div
                                                class="space-y-3 rounded-lg border border-slate-200 bg-white/70 p-3"
                                            >
                                                <h4
                                                ...
                                                <ul class="space-y-2">
                                                    {#each segment.assets as asset}
                                                        <li
                                                            class="rounded border border-slate-200 bg-white p-2 text-sm text-slate-600 flex flex-wrap items-center justify-between gap-2"
                                                        >
                                                            <div
                                                                class="space-x-2"
                                                            >
                                                                <span
                                                                    ...
                                                                    >{asset.file_name ??
                                                                        "Unnamed asset"}</span
                                                                >
                                                                {#if asset.mime_type}<span
                                                                        class="text-xs text-slate-500"
                                                                        >({asset.mime_type})</span
                                                                    >{/if}
                                                                {#if formatBytes(asset.size_bytes)}<span
                                                                        class="text-xs text-slate-500"
                                                                        >· {formatBytes(
                                                                            asset.size_bytes,
                                                                        )}</span
            ...
            {:else if viewMode === "sections"}
                    ...
                    <section class="space-y-4">
                        ...
                        >
                            ...
                            <span class="text-sm text-slate-600">
                                Segment {visibleSegments.length
                                    ? selectedSegmentIndex + 1
                                    : 0} of {visibleSegments.length}
                            </span>
                            <div class="flex flex-wrap gap-2">
                                <button
                                    type="button"
                        ...
                        <div
                            role="group"
                            class="card rounded-lg border border-slate-200 bg-white shadow-lg"
                        >
                            <div
                                class={`card-body space-y-6 ${currentSegment ? segmentAccentClass(currentSegment.source_role) : ""}`}
                            >
                                <header
                                ...
                                >
                                    ...
                                    >
                                        <span
                                            >{formatDate(
                                                currentSegment.started_at,
                                            )} · {currentSegment.segment_type}</span
                                        >
                                        <button
                                            class="btn btn-xs btn-outline"
                                            onclick={() =>
                                        ...
                                        {#each segmentedContent as part, index}
                                            ...
                                            {:else}
                                                ...
                                                >
                                                        ...
                                                        >
                                                            ...
                                                            <button
                                                                ...
                                                                onclick={() =>
                                                                    downloadCode(
                                                                        part.content,
                                                                        `segment-${currentSegment.sequence}-code-${part.snippet}.txt`,
                                                                    )}
                                                            >
                                                                Download
                                                            </button>
                                                    ...
                                                    <pre
                                                        class="code-block overflow-x-auto rounded-md border border-slate-200 bg-white p-4 text-sm leading-relaxed text-slate-900"><code
                                                            >{@html highlightCode(
                                                                part.content,
                                                                part.language,
                                                            ).trim()}</code
                                ...
                                {:else}
                                    <div class="markdown">
                                        {@html renderMarkdown(
                                            currentSegment.content_markdown,
                                        )}
                                    </div>
                                {/if}
                                            ...
                                            {#if block.block_type !== "markdown"}
                                                ...
                                                >
                                                    ...
                                                    {#if block.block_type === "code"}
                                                            ...
                                                            >
                                                                ...
                                                                <button
                                                                    ...
                                                                    onclick={() =>
                                                                        downloadCode(
                                                                            block.body,
                                                                            `segment-${currentSegment.sequence}-code-block-${block.sequence}.txt`,
                                                                        )}
                                                                >
                                                                    Download
                                                                </button>
                                                        ...
                                                        <pre
                                                            class="code-block overflow-x-auto rounded-md border border-slate-200 bg-slate-100 p-4 text-sm leading-relaxed text-slate-900"><code
                                                                >{@html highlightCode(
                                                                    block.body,
                                                                    block.language ??
                                                                        null,
                                    ...
                                    >
                                        ...
                                        <ul class="space-y-3">
                                            {#each currentSegment.assets as asset}
                                                <li
                                                    class="space-y-3 rounded-md border border-slate-200 bg-white p-3 text-sm"
                                                >
                                                    <div
                                                        class="flex flex-wrap items-center justify-between gap-3"
                                                    >
                ...
                {:else}
                    <div class="alert alert-info bg-slate-50 text-slate-600">
                        <span
                            >No segments to display. Try showing empty segments
                            or pick another document.</span
                        >
                    </div>
                {/if}
            {/if}
