<script lang="ts">
    ...
    async function run() {
        ...
        try {
            ...
            if (mode === "bm25") {
                ...
                if (threshold.trim())
                    u.searchParams.set("threshold", threshold);
                url = u.toString();
            } else {
            ...
            console.log(
                `[search] mode=${mode} query="${q}" fetched in ${Math.round(performance.now() - t0)}ms`,
            );
        } catch (e) {
            errorMsg = e instanceof Error ? e.message : "Unknown error";
            console.log(
                `[search] mode=${mode} query="${q}" failed after ${Math.round(performance.now() - t0)}ms`,
                e,
            );
        } finally {
            loading = false;
    ...
    async function explain() {
        ...
        try {
            ...
            plan = json;
            console.log(
                `[search-plan] mode=${mode} query="${q}" fetched in ${Math.round(performance.now() - t0)}ms`,
            );
        } catch (e) {
            planError = e instanceof Error ? e.message : "Unknown error";
            console.log(
                `[search-plan] mode=${mode} query="${q}" failed after ${Math.round(performance.now() - t0)}ms`,
                e,
            );
        } finally {
            planLoading = false;
...
<main class="min-h-screen space-y-6 bg-slate-50 p-6 text-slate-900">
    ...
    >
        <div class="flex flex-wrap items-center gap-3">
            ...
            <button
                ...
                disabled={loading || !q.trim()}>Search</button
            >
        </div>
        ...
        {#if mode === "bm25"}
            ...
            >
                <label
                    >Score threshold (optional): <input
                        class="input input-sm input-bordered bg-white w-32"
            ...
            >
                <label
                    >BM25 weight <input
                        ...
                        min="0"
                        max="1"
                        class="input input-sm input-bordered bg-white w-24"
                        bind:value={w_bm25}
                    /></label
                >
                <label
                ...
                <label
                    >k <input
                        type="number"
                        min="1"
                        class="input input-sm input-bordered bg-white w-20"
                        bind:value={k}
                    /></label
                >
            </div>
    ...
    <section
        class="rounded-xl border border-dashed border-slate-300 bg-white/60 p-4"
    >
        <h2 class="text-lg font-semibold text-slate-900">Instrumentation</h2>
        <p class="text-sm text-slate-600">
            Use "Explain query" to inspect planner output for the current
            mode/parameters.
        </p>
        <div class="mt-3 flex flex-wrap items-center gap-3">
            ...
            <label class="label cursor-pointer gap-2 text-slate-600">
                <input
                    ...
                    bind:checked={analyzePlan}
                />
                <span class="label-text text-xs">Analyze (executes query)</span>
            </label>
        </div>
        {#if planError}
            <div class="alert alert-error mt-3 bg-rose-50 text-rose-600">
                {planError}
            </div>
        {/if}
        {#if plan}
            <details
                class="mt-3 rounded-lg border border-slate-200 bg-white"
                open
            >
                <summary
                    class="flex items-center justify-between gap-3 cursor-pointer px-4 py-2 text-sm text-slate-700"
                >
                    <div>
                        <span class="font-semibold">Latest plan</span>
                        <span class="ml-2 text-xs text-slate-500"
                            >({plan.metadata?.mode})</span
                        >
                    </div>
                    <button
                        class="btn btn-xs"
                ...
                <div class="border-t border-slate-200 p-4">
                    <div class="grid gap-4 md:grid-cols-2">
                        <div
                            class="rounded-lg border border-slate-200 bg-white p-3 text-sm text-slate-600 space-y-1"
                        >
                            <div>
                            ...
                            <div>
                                ...
                                {plan.execution_time_ms ??
                                    (analyzePlan ? "n/a" : "plan only")}
                            </div>
                            <div>
                                <span class="font-semibold">Limit/Offset:</span>
                                {plan.metadata?.limit}/{plan.metadata?.offset}
                            </div>
                        </div>
                        <div
                            class="rounded-lg border border-slate-200 bg-white p-3 text-sm text-slate-600 overflow-auto"
                        >
                            <pre
                                class="text-xs whitespace-pre-wrap">{JSON.stringify(
                                    plan.metadata,
                                    null,
                                    2,
                                )}</pre>
                    ...
                    <div
                        class="mt-4 rounded-lg border border-slate-200 bg-slate-900/5 p-3 text-xs overflow-auto"
                    >
            ...
            {#each segmentResults as r (r.segment_id)}
                <li
                    ...
                    oncontextmenu={(event) => {
                        ...
                        downloadSegment(r.segment_id, r.document_title);
                    }}
                >
                    <div class="flex items-start justify-between gap-3">
                        <div class="text-sm text-slate-600">
                            <a
                                ...
                                href={`/?document=${r.document_id}&segment=${r.segment_id}`}
                                onclick={markDocumentNavigation}
                            >
                                {r.document_title}
                            </a>
                            <span class="ml-2 uppercase badge badge-outline"
                                >{r.source_system}</span
                            >
                        </div>
                        <div
                            class="text-xs text-slate-500 text-right space-y-0.5"
                        >
                            <div>
                                Updated {formatDate(r.document_updated_at)}
                            </div>
                            <div>
                                {r.document_segment_count ?? 0} segments · {formatChars(
                                    r.document_char_count,
                                )} chars
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="markdown mt-2 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm whitespace-pre-line wrap-break-word"
                    >
                    ...
                    >
                        {#if r.started_at}<span
                                >Started {formatDate(r.started_at)}</span
                            >{/if}
                        <button
    ...
    {:else if mode === "hybrid" && documentResults.length}
        <ul class="space-y-4">
            {#each documentResults as doc (doc.document_id)}
                <li
                    class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-sky-300"
                >
                    <div
                    ...
                    >
                        <div class="text-sm text-slate-600 space-y-1">
                            ...
                            <div class="space-x-2 text-xs text-slate-500">
                                <span class="uppercase badge badge-outline"
                                    >{doc.source_system}</span
                                >
                                <span
                                    >Score {doc.document_score.toFixed(3)}</span
                                >
                                <span
                        ...
                        >
                            <div>
                                Updated {formatDate(doc.document_updated_at)}
                            </div>
                            <div>
                                {doc.document_segment_count ?? 0} segments · {formatChars(
                                    doc.document_char_count,
                                )} chars
                            </div>
                        </div>
                    </div>
                    <div class="mt-3 space-y-3">
                        ...
                        {:else}
                            {#each doc.top_segments as segment, index (segment.segment_id + "-" + index)}
                                {#if doc.top_segments.findIndex((s) => s.segment_id === segment.segment_id) === index}
                                    ...
                                    >
                                        <div
                                            class="flex flex-wrap items-center justify-between text-xs uppercase tracking-wide text-slate-500 gap-2"
                                        >
                                            <span
                                                >{segment.sequence} · {segment.source_role}</span
                                            >
                                            <div
                                                class="flex items-center gap-2"
                                            >
                                                <span
                                                    >Score {segment.score.toFixed(
                                                        3,
                                                    )}</span
                                                >
                                                <button
                                        ...
                                        <div
                                            class="markdown text-sm whitespace-pre-line wrap-break-word"
                                        >
                                        ...
                                        >
                                            ...
                                            <a
                                                ...
                                                href={`/?document=${doc.document_id}&segment=${segment.segment_id}`}
                                                onclick={markDocumentNavigation}
                                                >Open in viewer</a
                                            >
                                        </div>
                        ...
                        {#if doc.match_count > doc.top_segments.length}
                            <div class="text-xs text-slate-500">
                                +{formatNumber(
                                    doc.match_count - doc.top_segments.length,
                                )} more matching segment{doc.match_count -
                                    doc.top_segments.length ===
                                1
                                    ? ""
                                    : "s"} not shown.
                            </div>
