...
export const actions = {
	default: async ({ request, locals, fetch }) => {
		...
		try {
			...
			return { success: true, message: result.message ?? 'Ingestion started successfully.' };

		} catch (err) {
			console.error('Upload action error:', err);
			return fail(500, { error: 'Internal server error while contacting ingestion service.' });
		}
	}
} satisfies Actions;
